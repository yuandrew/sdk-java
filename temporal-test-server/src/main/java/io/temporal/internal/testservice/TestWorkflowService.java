...
public final class TestWorkflowService extends WorkflowServiceGrpc.WorkflowServiceImplBase
  ...
  public void describeWorker(
      DescribeWorkerRequest request,
      StreamObserver<DescribeWorkerResponse> responseObserver) {
    ...
    if (hb == null) {
      responseObserver.onError(
          Status.NOT_FOUND
              .withDescription(
                  "Worker not found: " + request.getWorkerInstanceKey())
              .asRuntimeException());
      return;
    }
    responseObserver.onNext(
        DescribeWorkerResponse.newBuilder()
            .setWorkerInfo(
                io.temporal.api.worker.v1.WorkerInfo.newBuilder()
                    .setWorkerHeartbeat(hb)
                    .build())
            .build());
    responseObserver.onCompleted();
  }
